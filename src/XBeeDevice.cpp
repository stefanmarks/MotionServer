...
{
	std::cout << "Checking Serial Ports..." << std::endl;
	for (int i = 20; i < 50; i++)
	{
		SerialPort serialPort(i);
		if (serialPort.exists())
		{
			std::cout << "Found COM" << i << std::endl;
			if (serialPort.open())
			{
				XBeeCoordinator device(serialPort);
				if (device.isValid())
				{
					std::cout
						<< "XBee Coordinator '" << device.getName() << "' found. "
						<< "Serial# " << std::hex << device.getSerialNumber()
						<< ", Address " << std::hex << device.getNetworkAddress()
						<< ", Version " << std::hex << device.getSoftwareVersion() << " SW"
						<< ", " << std::hex << device.getHardwareVersion() << " HW"
						<< std::endl;
					...
					for (int i = 0; i < 100; i++)
					{
						std::unique_ptr<XBeePacket_Receive> packet = device.receive();
						if (packet)
						{
							if (packet->getFrameTypeID() == XBeePacket_IO_DataSample::FRAME_TYPE_ID)
							{
								...
								std::cout << "IO sample from " << std::hex << sample->getSerialNumber()
									<< ", Addr " << std::hex << sample->getNetworkAddress()
									<< ", Mask " << std::hex << sample->getDigitalInputMask()
									<< ", State " << std::hex << sample->getDigitalInputState()
									<< std::endl;

								XBeePacket_RemoteAT_Command rcmd("NI");
								rcmd.setSerialNumber(sample->getSerialNumber());
								rcmd.setNetworkAddress(sample->getNetworkAddress());
								XBeePacket_RemoteAT_CommandResponse rr;
								if (device.process(rcmd, rr))
								{
									std::cout << "Remote device '" << rr.getString() << "'" 
										<< std::endl;
								}
							}
							else
							{
								std::cout << "received packet with frame type "
									<< std::hex << packet->getFrameTypeID()
									<< std::endl;
							}
						}
					}
